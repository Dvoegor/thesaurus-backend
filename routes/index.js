        ...
        pool.query(`UPDATE crossWords SET question='${req.body.question}', answer='${req.body.answer}', function='${req.body.function}', method='${req.body.method}', subjectArea='${req.body.subjectArea}' WHERE id=${req.body.id}`,
            function (err, results) {
                if (err) {
                    ...
                    pool.query(`INSERT INTO logs (record_id,email,time,date,log_type)
                VALUES (
                '${req.body.id}',
                '${req.session.email}',
                '${time}',
                ...
                `,
                        function (err, results) {
                            if (err) {
                                res.status(500).send({message: 'Ошибка добавления логов', status: 'error'})
                            } else {
        ...
        pool.query(`INSERT INTO crossWords (question,answer,function,method,subjectArea)
            ...
            function (err, results) {
                if (err) {
                    ...
                    pool.query(`SELECT COUNT(*) FROM crossWords`,
                        function (err, data) {
                            if (err) {
                                console.log(err);
                            } else {
